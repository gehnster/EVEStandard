...
namespace EVEStandard.API
{
    ...
    public class CorporationProjects : APIBase
    {
        ...
        /// <para>GET /corporations/{corporation_id}/projects/{project_id}/contributors/</para>
        /// <para>Requires one of the following EVE corporation role(s): Project_Manager</para>
        /// <para>This endpoint uses cursor-based pagination.</para>
        /// </summary>
        /// <param name="auth">The <see cref="AuthDTO"/> object.</param>
        /// <param name="corporationId">An EVE corporation ID.</param>
        ...
        /// <param name="ifNoneMatch">ETag from a previous request. A 304 will be returned if this matches the current ETag.</param>
        /// <returns><see cref="ESIModelDTO{T}"/> containing a list of project contributors.</returns>
        public async Task<ESIModelDTO<CorporationProjectContributors>> GetCorporationProjectContributorsAsync(AuthDTO auth, long corporationId, string projectId, string after = null, string before = null, int limit = 10, string ifNoneMatch = null)
        {
            CheckAuth(auth, Scopes.ESI_CORPORATIONS_READ_PROJECTS_1);
            ...
            var responseModel = await GetAsync($"/corporations/{corporationId}/projects/{projectId}/contributors/", auth, ifNoneMatch, queryParameters);

            CheckResponse(nameof(GetCorporationProjectContributorsAsync), responseModel.Error, responseModel.Message, responseModel.LegacyWarning, logger);

            return ReturnModelDTO<CorporationProjectContributors>(responseModel);
        }
    }
}
