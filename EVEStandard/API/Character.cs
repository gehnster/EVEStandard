...
namespace EVEStandard.API
{
    ...
    public class Character : APIBase
    {
        ...
        public async Task<ESIModelDTO<List<Notification>>> GetCharacterNotificationsV3Async(AuthDTO auth, string ifNoneMatch = null)
        {
            CheckAuth(auth, Scopes.ESI_CHARACTERS_READ_NOTIFICATIONS_1);

            var responseModel = await GetAsync($"/v3/characters/{auth.CharacterId}/notifications/", auth, ifNoneMatch);

            CheckResponse(nameof(GetCharacterNotificationsV3Async), responseModel.Error, responseModel.Message, responseModel.LegacyWarning, logger);
