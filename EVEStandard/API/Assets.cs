...
namespace EVEStandard.API
{
    ...
    public class Assets : APIBase
    {
        ...
        public async Task<ESIModelDTO<List<Asset>>> GetCharacterAssetsV5Async(AuthDTO auth, int page = 1, string ifNoneMatch = null)
        {
            ...
            var responseModel = await GetAsync($"/v5/characters/{auth.CharacterId}/assets/", auth, ifNoneMatch, queryParameters);

            CheckResponse(nameof(GetCharacterAssetsV5Async), responseModel.Error, responseModel.Message, responseModel.LegacyWarning, logger);
